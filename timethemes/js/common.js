...
function checkTime() {
	...
	let noonSplit = localStorage.getItem(noonTimeKey).split(":");
    ...
    if (timeInBetween(
			...
			noonSplit[0], noonSplit[1], 
            
        ){
        enableTheme(localStorage.getItem(daytimeThemeKey));
    } 
	else if (timeInBetween(
			hours, minutes, 
			noonSplit[0], noonSplit[1], 
            sunsetSplit[0], sunsetSplit[1])
		){
        enableTheme(localStorage.getItem(noontimeThemeKey));
    }
...
function timeInBetween(
	...
	let noonInMins = noonHours * 60 + parseInt(noonMins);
